...
## 步骤 1：确认审查范围
使用 `view_file` 工具读取 `.agents/skills/requirement-clarify-java/SKILL.md`，按照「code-review 专属问题清单」确认审查范围和背景。
...
## 步骤 3：系统审查
使用 `view_file` 工具读取 `.agents/skills/code-review-java/SKILL.md`，按照三维度审查流程逐项检查：
- 🔴 安全审查（优先级最高）
- 🟡 规范审查
- 🟢 质量审查

## 步骤 4：输出审查报告
按照 `code-review-java` Skill 的报告模板输出结果。
