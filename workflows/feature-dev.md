...
## 步骤 1：需求澄清
使用 `view_file` 工具读取 `.agents/skills/requirement-clarify/SKILL.md`，按照「feature-dev 专属问题清单」执行需求澄清流程。

**在所有 🔴 必问项（F-01 ~ F-04）都已确认之前，禁止进入下一步。**

## 步骤 2：方案设计
使用 `view_file` 工具读取 `.agents/skills/feature-dev/SKILL.md`，按照阶段一输出实现方案：
- 影响范围、文件清单
- 数据库变更、接口设计
- 核心业务逻辑、安全考量
...
按照 `feature-dev` Skill 阶段二的分层顺序编码：
