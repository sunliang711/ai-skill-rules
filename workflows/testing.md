...
## 步骤 1：确认测试范围
使用 `view_file` 工具读取 `.agents/skills/requirement-clarify/SKILL.md`，按照「testing 专属问题清单」确认测试范围和类型。

**在所有 🔴 必问项（T-01 ~ T-02：测试范围、测试类型）都已确认之前，禁止进入下一步。**

## 步骤 2：设计测试用例
使用 `view_file` 工具读取 `.agents/skills/testing/SKILL.md`，按照步骤 1~2 分析目标代码并输出测试用例设计表。
...
按照 `testing` Skill 步骤 3 的规范编写测试代码。
...
## 步骤 5：输出交付
按照 `testing` Skill 步骤 4 输出测试完成摘要。
